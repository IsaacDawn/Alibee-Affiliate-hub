...
export function DesktopProductCard({
  ...
  return (
    <div className="relative bg-gradient-to-l from-purple-600 to-pink-600 overflow-hidden min-h-screen mt-16 pt-8">
      {/* Responsive Layout - Side by Side */}
      <div className="relative flex flex-col lg:flex-row">

        {/* Left Side - Product Images (responsive width, top-left aligned, square aspect ratio) */}
        <div className="w-full lg:w-1/3 flex items-start justify-start p-2 sm:p-3 lg:p-4">
        ...
        {/* Right Side - Product Information (responsive width) */}
        <div className="w-full lg:w-2/3 flex flex-col text-white pt-8 sm:pt-10 lg:pt-12 pr-2 sm:pr-3 lg:pr-4 pb-2 sm:pb-3 lg:pb-4 pl-2 sm:pl-3 lg:pl-4">
          <div className="flex-1 space-y-3 sm:space-y-4 lg:space-y-6">
            ...
            <div className="space-y-2 sm:space-y-3 lg:space-y-4">
              {isEditingTitle ? (
                <div className="relative">
                  <input
                    type="text"
                    className="w-full bg-transparent border-2 border-white border-dashed text-white text-xl sm:text-2xl lg:text-4xl font-bold outline-none px-2 sm:px-3 lg:px-4 py-1 sm:py-2 rounded"
                  ...
                  <div className="flex space-x-1 sm:space-x-2 mt-1 sm:mt-2">
                    <button
                      ...
                      className="px-2 sm:px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-xs rounded transition-colors"
                    >
                      ✓ Save
                    </button>
                    <button
                      onClick={handleTitleCancel}
                      className="px-2 sm:px-3 py-1 bg-gray-500 hover:bg-gray-600 text-white text-xs rounded transition-colors"
                    >
                      ✗ Cancel
                    </button>
                  </div>
                ...
                <h1 
                  className="text-white text-xl sm:text-2xl lg:text-4xl font-bold cursor-pointer hover:bg-white/10 px-2 sm:px-3 lg:px-4 py-1 sm:py-2 rounded transition-colors" 
                  style={{ textShadow: '2px 2px 4px black' }}
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 lg:space-x-4">
              <p className="text-lg sm:text-xl lg:text-3xl font-semibold text-green-400">
                {formattedPrice}
              </p>
              {formattedOriginalPrice && (
                <span className="text-sm sm:text-base lg:text-xl text-gray-300 line-through">
                  {formattedOriginalPrice}
                </span>
              )}
            </div>
                    ...
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <div key={star} className="relative">
                            <StarIcon 
                              className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 opacity-30" 
                              filled={false} 
                              halfFilled={false}
                            />
                          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 lg:gap-4 text-xs sm:text-sm lg:text-base text-gray-300">
              <div className="flex items-center space-x-1 sm:space-x-2">
            ...
            <div className="flex justify-end">
              <button
                ...
                onClick={toggleSave}
                className={`w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 rounded-full flex items-center justify-center transition-all duration-300 ${
                  saved
                    ? "text-red-500 bg-white/20"
                    : "text-white bg-white/10 hover:bg-white/20"
                } ${isSearchInterfaceOpen ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                <HeartIcon className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8" filled={saved} />
              </button>
            </div>
          </div>
          
          {/* Buy Button - Centered horizontally, aligned with image bottom */}
          <div className="flex justify-center items-end">
            <button 
              className={`bg-red-500 hover:bg-red-600 text-white px-4 py-2 sm:px-6 sm:py-3 lg:px-10 lg:py-5 rounded-full text-sm sm:text-base lg:text-xl font-bold transition-all duration-300 transform hover:scale-105 shadow-lg ${isSearchInterfaceOpen ? "opacity-50 cursor-not-allowed" : ""}`}
